...
${description}
    ...
    // Trigger Welcome Email if Resend is configured
    if (resend && session.user.email) {
      try {
        await resend.emails.send({
          from: DEFAULT_SENDER,
          to: session.user.email,
          subject: `Welcome to N3xUs - ${project.title}`,
          react: WelcomeEmail({ name: session.user.name || 'Client', projectName: project.title }),
        });
      } catch (emailError) {
